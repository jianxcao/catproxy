...
import { addBranch, addGroup, disableAll, disCache, cacheFlush } from './action/actions';
...
class SubHeader extends React.Component {
	...
	handleSaveHosts() {
		...
		sendMsg.updateRule(this.props.hosts.toJS()).then(
			message => {
	...
	handleCacheFlush(proxy, status) {
		...
		sendMsg.cacheFlush(status).then(
			message => {
				com.context.toast(message.result);
			},
			message => com.context.toast(message.result)
	...
	renderDialog() {
		...
		const actions = [
			<FlatButton label='取消' primary={true} onTouchTap={this.handleCloseDialog} />,
			<FlatButton label='保存' primary={true} keyboardFocused={true} onTouchTap={this.handleAddBranch} />,
		];
		let source = (this.groups = this.props.hosts
			.map((current, index) => {
				return {
			...
			.toJS());
		return (
			<Dialog actions={actions} modal={true} open={this.state.openDialog} contentStyle={contentStyle} autoScrollBodyContent={true} bodyStyle={bodyStyle}>
				<AutoComplete
				...
				/>
				<br />
				<TextField ref='branchEel' floatingLabelText='规则名称' type='text' onKeyUp={this.handleEnterKeyAddBranch} onChange={this.handleChangeBranch} />
			</Dialog>
		);
	}
