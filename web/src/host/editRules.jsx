...
import { addRule, delRule, toggleRuleDis, updateRule, switchRule } from './action/actions';
	...
	shouldComponentUpdate(nextProps, nextState) {
		...
		return (
			...
			rules.some((current, index) => {
				...
				return curRules.get('disable') !== current.get('disable') || curRules.get('type') !== !curRules.get('type');
			})
		);
	}
	// 新建一个规则
	...
	render() {
		...
		if (groupId !== null && branchId !== null && groupId >= 0 && branchId >= 0) {
			...
			let editRules = rules.map((current, index) => {
				return <EditRule rule={current} key={index} ruleId={index} delRule={this.delRule} disRule={this.disRule} updateRule={this.updateRule} switchProps={this.switchProps} />;
			});
			return (
				<Paper zDepth={0}>
