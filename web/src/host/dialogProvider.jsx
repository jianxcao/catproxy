...
export default class DialogProvider extends React.Component {
	...
	getBtns(btns = [], dialogId) {
		...
		return btns.map((current, index) => {
			...
			return <FlatButton key={index} label={text} primary={true} data-btn-id={index} data-dialog-id={dialogId} keyboardFocused={text !== current} onTouchTap={this.handleBtnClick} />;
		});
	}
