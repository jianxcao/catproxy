...
export default class ResToolBar extends Component {
	...
	render() {
		...
		let lan = allLanguage.map(current => (
			<MenuItem eventKey={current} key={current}>
				{current}
			</MenuItem>
		));
		...
		result['bar2'] = (
			<Nav>
				<NavDropdown eventKey={'language'} title={language} className='language' id='language' onSelect={this._changeLanguage}>
					{lan}
				</NavDropdown>
			</Nav>
		);
		...
		result['bar4'] = (
			<div className='formatCode'>
				<span>
					<input type='checkbox' name='type' id='resToolBarFormatCode' value='format' defaultChecked={!!formatCode} onChange={this._changeFormat} />
					<label htmlFor='resToolBarFormatCode'>美化</label>
				</span>
			</div>
		);
