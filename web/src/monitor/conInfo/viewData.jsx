...
export default class ViewData extends Component{
	...
	render() {
		...
		if (header) {
			...
			result.push(<div key="header" className={headerCls} onClick={this._handleClickTitle}>{header}</div>);
		}
		let contentStyle = {
		...
		if (content || children) {
			result.push(<div key="content" className={contentCls} style={contentStyle}>{content || ""}{children}</div>);
		}
		let warpCls = cx('viewData', 'viewHeader', ... (className || "").split(" "));
		return (
			<div {...props} className={warpCls}>{result}</div>
		);
	};
}
