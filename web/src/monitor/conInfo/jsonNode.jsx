...
export default class JsonNode extends Component{
	...
	render() {
		...
		let dotted = [];
		if (propKey) {
			dotted.push(<span className="key" key="propKey">{propKey}</span>);
			dotted.push(':');
		}
		if (type === 'array') {
			dotted.push(<span className="abbr" key="abbr">{isExpand ? "[" : "[ ... ]"}</span>);
		} else {
			dotted.push(<span className="abbr" key="abbr">{isExpand ? "{" : "{ ... }"}</span>);
		}
		...
		return (
			<div {...props} className={warpCls}>
				<div key="header" className={headerCls} onClick={this._handleClickTitle}><span className="start">{dotted}</span></div>
				<div style={contentStyle}>
					<div key="content" className={contentCls}>{children}</div>
