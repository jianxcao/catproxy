...
export default class ViewHeader extends Component{
	...
	render() {
		...
		let result = [];
		let headers = [header];
		headers.push(<em className="encode" key= "source" onClick={this._handleEncodeClick}>{isUrlDecode ? "源编码" : "解码"}</em>);
		let t = typeof content;
		if (content) {
			if (content.size && content.reduce) {
				result = content.reduce((all, current, index) => {
					all.push(<div key={all.length}><span>{index}:&nbsp;</span><em>{isUrlDecode ? decodeURIComponent(current) : current}</em></div>);
					return all;
				}, []);
			} else if (t === 'object'){
				result = [];
				for(let key in content) {
					result.push(<div key={result.length}><span>{key}:&nbsp;</span><em>{isUrlDecode ? decodeURIComponent(content[key]) : content[key]}</em></div>);
				}
			}
		}
		return (
