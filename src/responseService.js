...
let detailHost = function(result, reqInfo, resInfo) {
	...
	return changeHost(result.hostname, isServerPort).then(
		address => {
			// 如果还是死循环，则跳出
		...
		err => {
			let { res } = resInfo;
			return triggerBeforeRes.call(com, merge(resInfo, { statusCode: 504 }, { bodyDataErr: err, headers: {} })).then(() => {
				return Promise.reject(err);
			});
		}
	);
...
let proxyReq = function(options, reqInfo, resInfo, req) {
	...
	return new Promise((resolve, reject) => {
		...
		let proxyReq = (isStartHttps.test(reqInfo.protocol) ? https : http).request(options, proxyRes => {
				...
				.on('end', () => {
					...
					return Promise.resolve(bodyData)
						.then(bodyData => {
							// 文件大小没有出错的情况下
							if (!isError) {
								return triggerBeforeRes.call(com, merge(resInfo, { bodyData })).then(
									resInfo => {
		...
		.then(null, err => {
			...
			// 出错也要处理下
			return triggerBeforeRes.call(com, merge({ statusCode }, resInfo, { bodyDataErr: err, headers: {} })).then(() => {
				return Promise.reject(err);
			});
		});
};
		...
		Promise.resolve(beforeReq.call(self, reqInfo))
			...
			.then(({ reqInfo, resInfo }) => {
				...
				if (res.finished) {
					...
					resInfo.bodyData = '';
					triggerBeforeRes.call(self, resInfo).then(
						function() {
